    ...
    try {
        ...
        const fs::path tempPath = path.string() + ".tmp";
        {
            ...
            stream << std::fixed << std::setprecision(3)
                   ...
                   << "    \"root_path\": \"" << jsonEscape(status.storageRootPath) << "\",\n"
                   << "    \"status_output_path\": \"" << jsonEscape(status.storageStatusOutputPath) << "\",\n"
                   << "    \"dashboard_output_path\": \"" << jsonEscape(status.storageDashboardOutputPath) << "\",\n"
                   << "    \"log_file_path\": \"" << jsonEscape(status.storageLogFilePath) << "\",\n"
                   << "    \"log_max_bytes\": " << status.storageLogMaxBytes << ",\n"
                   << "    \"log_backup_count\": " << status.storageLogBackupCount << ",\n"
                   << "    \"log_rotation_failure_count\": " << status.storageLogRotationFailureCount << ",\n"
                   << "    \"log_write_failure_count\": " << status.storageLogWriteFailureCount << ",\n"
                   << "    \"log_last_error\": \"" << jsonEscape(status.storageLogLastError) << "\",\n"
                   << "    \"history_enabled\": " << (status.historyEnabled ? "true" : "false") << ",\n"
                   << "    \"history_output_path\": \"" << jsonEscape(status.historyOutputPath) << "\",\n"
                   << "    \"last_error\": \"" << jsonEscape(status.storageLastError) << "\"\n"
                   << "  },\n"
                   << "  \"gnss\": {\n"
                   ...
                   << "  },\n"
                   << "  \"sensor_hub_diagnostics\": {\n"
                   << "    \"seen\": " << (mcu.diagnostics.seen ? "true" : "false") << ",\n"
                   << "    \"schema_version\": "
                   << static_cast<unsigned int>(mcu.diagnostics.schemaVersion) << ",\n"
                   << "    \"uptime_ms\": " << mcu.diagnostics.uptimeMs << ",\n"
                   << "    \"i2c_recovery_count\": " << mcu.diagnostics.i2cRecoveryCount << ",\n"
                   << "    \"i2c_transaction_failure_count\": "
                   << mcu.diagnostics.i2cTransactionFailureCount << ",\n"
                   << "    \"i2c_last_hal_error\": " << mcu.diagnostics.i2cLastHalError << ",\n"
                   << "    \"fifo_overflow_count\": " << mcu.diagnostics.fifoOverflowCount << ",\n"
                   << "    \"fifo_malformed_packet_count\": "
                   << mcu.diagnostics.fifoMalformedPacketCount << ",\n"
                   << "    \"fifo_empty_event_count\": "
                   << mcu.diagnostics.fifoEmptyEventCount << ",\n"
                   << "    \"fifo_drain_stall_count\": "
                   << mcu.diagnostics.fifoDrainStallCount << ",\n"
                   << "    \"fifo_skipped_packet_count\": "
                   << mcu.diagnostics.fifoSkippedPacketCount << ",\n"
                   << "    \"i2c_last_length\": " << mcu.diagnostics.i2cLastLength << ",\n"
                   << "    \"i2c_last_device_address\": "
                   << static_cast<unsigned int>(mcu.diagnostics.i2cLastDeviceAddress) << ",\n"
                   << "    \"i2c_last_register_address\": "
                   << static_cast<unsigned int>(mcu.diagnostics.i2cLastRegisterAddress) << ",\n"
                   << "    \"i2c_last_operation\": "
                   << static_cast<unsigned int>(mcu.diagnostics.i2cLastOperation) << ",\n"
                   << "    \"i2c_last_hal_status\": "
                   << static_cast<unsigned int>(mcu.diagnostics.i2cLastHalStatus) << ",\n"
                   << "    \"icm42688_init_error_step\": "
                   << static_cast<unsigned int>(mcu.diagnostics.icm42688InitErrorStep) << ",\n"
                   << "    \"uart4_rx_drop_count\": "
                   << mcu.diagnostics.uart4RxDropCount << "\n"
                   << "  },\n"
                   << "  \"barometer\": {\n"
                   ...
                   << "    \"tilt_compensated\": "
                   << (compass.tiltCompensated ? "true" : "false") << ",\n"
                   << "    \"imu_uptime_ms\": " << compass.imuUptimeMs << ",\n"
                   << "    \"magnetometer_uptime_ms\": " << compass.magnetometerUptimeMs << ",\n"
                   << "    \"magnetic_heading_degrees\": "
                   << compass.magneticHeadingDegrees << ",\n"
                   << "    \"heading_degrees\": " << compass.headingDegrees << ",\n"
                   << "    \"roll_degrees\": " << compass.rollDegrees << ",\n"
                   << "    \"pitch_degrees\": " << compass.pitchDegrees << ",\n"
                   << "    \"field_strength_ut\": " << compass.fieldStrengthMicroTesla << ",\n"
                   << "    \"quality\": \"" << jsonEscape(compass.quality) << "\",\n"
                   << "    \"last_error\": \"" << jsonEscape(compass.lastError) << "\"\n"
                   << "  },\n"
                   << "  \"imu\": {\n"
