...
F407 侧 ICM42688 已完成 100 Hz 真实数据上报验证，但 F407 -> MP157 的真实串口链路仍未联调。MP157 开发板自身带有 ICM20608，正点原子资料中提供了字符设备、regmap 和 IIO 三类 SPI 示例。
...
F407 -> MP157 真实串口对通暂后移到 MP157 板载 ICM20608 上板验证之后。
...
- 完成 F407 -> MP157 串口输入源后，将外部 Sensor Hub 数据与板载 IMU 数据同时纳入 Runtime 状态。
