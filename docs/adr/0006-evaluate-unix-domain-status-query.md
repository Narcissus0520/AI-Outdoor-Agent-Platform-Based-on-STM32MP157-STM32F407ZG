...
2026-07-21：Stage 2.1 已在保留文件型状态发布的同时实现可选 Unix domain socket 查询。非阻塞调度、最小行协议、资源上限、权限、stale/active socket 处理、查询客户端和剩余 MP157 验收边界见 [ADR-0025](0025-use-unix-domain-socket-status-query.md)。本 ADR 保留 Stage 0 当时“先评估、后实现”的范围决策，不覆盖历史背景。
