...
当前已完成 F407 侧 MMC5603 I2C2 数据源、20 Hz 磁力计帧和 MP157 parser/status 接入，并通过真实磁场数据历史上板验证；当前 MMC5603 已按 I2C 排障需要断开。BMP390 气压/温度软件路径、10 Hz 协议帧和 MP157 parser/status 已实现并通过构建和烧录链路验证，但真实模块当前在 `0x76/0x77` 均未 ACK。ICM42688 当前重新接线后仍处于 Mock fallback，最新 heartbeat 为 `status_flags=0x0056`，待单独排查。UBLOX-M10 已确认 MP157 UART5 `/dev/ttySTM2`、38400 8N1 可收到有效 NMEA；当前无卫星信号。
...
- [ ] 完成 BMP390 真实 I2C ACK、补偿气压/温度和 10 Hz `sensor_barometer` 帧上板验证
...
- 当前 Stage 1 保留 ICM42688 优先、Mock IMU 兜底策略；当前板上实际处于 Mock fallback，最新 I2C 隔离验证 heartbeat 为 `0x0056`。
- STM32F407ZG CubeMX 基础工程、仓库自主管理的 GNU ARM 构建和 UART Bootloader 上板验证已经完成。
- ICM42688 I2C 固件路径已实现并通过构建；真实传感器数据曾在修正接线后通过 F407 串口抓包确认，当前重新接线状态仍需排查。
- 当前 F407 固件已在板上通过 UART4 输出 heartbeat 和 Mock IMU 帧；MMC5603 曾完成真实磁力计帧验证，当前为隔离 I2C 问题已断开。
