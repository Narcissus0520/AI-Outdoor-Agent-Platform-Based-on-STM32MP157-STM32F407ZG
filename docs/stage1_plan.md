...
当前在 STM32Cube 固件基线和 MP157 Linux Runtime 协议解析基础上，已经完成 F407 侧 ICM42688 I2C 数据源接入。F407 固件已实现真实 ICM42688 优先、Mock IMU 兜底的数据源策略；真实传感器读数已在修正 SCL/SDA 接线后完成 F407 上板串口抓包验证。MP157 开发板自带 ICM20608 的主控侧传感器读取和上板验证已完成；F407 -> MP157 已按 `F407 UART4 PC10/PC11 <-> MP157 USART3 PD9/PD8` 完成最小板间联调。MP157 -> F407 下行 `command_ping -> command_ack` 软件路径已实现，并已通过 raw shell 方式完成物理链路验证。UBLOX-M10 当前已完成 MP157 UART5/NMEA 软件路径和文本仪表盘原型，尚未进行 UART5 上板验证。
...
- [ ] 上板确认 UBLOX-M10 当前波特率和 NMEA 输出语句集合
- [ ] 完成 UART5 真实接线验证
...
- UBLOX-M10 UART5 当前只完成 MP157 软件路径和 PC 自动化验证，尚未做真实上板验证。
- 当前 `outdoor-agent` 已支持 7 英寸 RGB 屏 framebuffer 仪表盘显示，但仍是轻量 fbdev 原型，不包含触摸/窗口系统/复杂 GUI 控件。
