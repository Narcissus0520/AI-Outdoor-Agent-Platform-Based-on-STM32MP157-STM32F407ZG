...
当前处于 Stage 1：Sensor Integration 与 STM32F407ZG Sensor Hub 协议解析。MP157 板载 ICM20608 的 Linux 用户态读取和上板验证已经完成；F407 -> MP157 串口输入已按 `F407 UART4 PC10/PC11 <-> MP157 USART3 PD9/PD8` 完成最小上板验收。MP157 -> F407 方向当前实现最小 `ping -> ack`，并已通过 MP157 shell raw 帧方式完成真实接线验证；新版 ARM Runtime `--mcu-command ping` 板端复测仍待完成。UBLOX-M10 当前已完成 MP157 UART5/NMEA 软件路径，尚未进行 UART5 上板验证。`outdoor-agent` 当前已可在 7 英寸 RGB 屏 `/dev/fb0` 上显示参考图风格的高级感仪表盘和 AI Agent 预留区。当前板上实测 ICM20608 默认入口为 `/dev/icm20608` 字符设备，IIO 路径保留为可选来源。
...
MP157 UART5 GNSS path
  ...
  -> /dev/ttySTM2, default 9600 8N1, raw mode
  -> services::GnssSerialService
  -> line buffer
  -> NmeaParser::parseLine()
  -> GnssStatus
...
UART5 NMEA 输入与文本仪表盘原型见 `docs/adr/0010-use-nmea-uart5-text-dashboard.md`。
...
- 当前 UBLOX-M10 UART5 只完成软件路径，尚未上板确认 `/dev/ttySTM2`、波特率和 NMEA 输出集合
