...
- `outdoor-agent` 仪表盘 APP 原型 `DashboardService`，支持 `runtime/dashboard.txt` 文本输出和 Linux fbdev `/dev/fb0` framebuffer 输出；framebuffer 视觉布局参考深色科技风户外终端仪表盘，包含左侧导航、顶部状态、方向/速度/位置/环境展示和底部状态栏
...
- F407 I2C2 读取 ICM42688 的固件数据源，具备历史上板验证记录，当前重新接线后仍回退 Mock IMU
- F407 I2C2 读取 MMC5603 三轴磁场，20 Hz `sensor_magnetometer` 帧已完成历史上板验证；当前为 I2C 排障已断开
- F407 I2C2 读取 BMP390 气压和温度，10 Hz `sensor_barometer` 软件路径已实现并通过构建/烧录链路验证，当前真实模块未 ACK
...
当前处于 Stage 1：Sensor Integration 与 STM32F407ZG Sensor Hub 协议解析。MP157 板载 ICM20608、F407 侧 MMC5603、F407/MP157 UART 链路均已完成真实上板验证。MMC5603 已通过 I2C2 地址 `0x30` 采样并以 20 Hz 上报，当前为 I2C 排障已断开；BMP390 软件路径已完成，启动时探测 `0x76/0x77` 并以 10 Hz 上报，但当前真实模块未 ACK；ICM42688 当前重新接线后仍处于 Mock fallback，最新 heartbeat `status_flags=0x0056`，待独立排查。UBLOX-M10 已确认在 `/dev/ttySTM2`、38400 8N1 输出有效 NMEA，但当前无卫星定位。
...
Dashboard path
  ...
  -> reference-style dark dashboard layout with navigation/sidebar/gauges/map/footer
...
MP157 SD card storage path
  ...
  -> logs/outdoor_core_runtime.log
  -> future sensor history files under data/ or captures/
...
BMP390 I2C sensor path
  ...
  -> software and build validation complete; board validation pending
...
- 配置格式为简单 `key=value`
- 当前支持 `gnss_input_mode`、`nmea_input_path`、`gnss_serial_device`、`gnss_serial_baud`、`gnss_serial_capture_seconds`、`mcu_input_mode`、`mcu_mock_input_path`、`mcu_serial_device`、`mcu_serial_baud`、`mcu_serial_capture_seconds`、`mcu_command`、`status_output_path`、`storage_enabled`、`storage_root_path`、`storage_status_output_path`、`storage_dashboard_output_path`、`storage_log_file_path`、`dashboard_enabled`、`dashboard_output_path`、`dashboard_output_mode`、`dashboard_framebuffer_device`、`dashboard_refresh_count`、`dashboard_refresh_interval_ms` 和 `log_level`
...
- 命令行参数可覆盖配置文件中的输入路径、日志级别和 storage root；`--storage-root` 会启用 SD storage
...
- `sensor_hub_app_init()` 同时初始化 MMC5603 和 BMP390；BMP390 自动探测 `0x77/0x76` 并配置 25 Hz normal mode。
- `sensor_hub_app_poll()` 负责 UART4 RX 命令轮询、500 ms LED 心跳、1 Hz heartbeat 帧、100 Hz IMU、20 Hz 磁力计和 10 Hz 气压计帧发送。
...
UART5 NMEA 输入与文本仪表盘原型见 `docs/adr/0010-use-nmea-uart5-text-dashboard.md`。BMP390 驱动选型见 `docs/adr/0011-use-bosch-bmp3-driver.md`。MP157 SD 卡文件持久化见 `docs/adr/0012-use-sd-card-file-storage.md`。
...
- F407 固件具备 ICM42688 100 Hz 读取和 Mock fallback；该路径曾通过 `status_flags=0x0001` 验证，但当前接线状态为 fallback/error，最新隔离抓包为 `status_flags=0x0056`。
- F407 固件能读取 MMC5603 并以约 20 Hz 输出 `sensor_magnetometer`，MP157 可解析 XYZ μT 状态；当前硬件为 I2C 排障已断开。
- F407 固件 BMP390 软件路径能生成 10 Hz `sensor_barometer`，MP157 可解析气压和温度；当前真实模块在 `0x76/0x77` 均未 ACK。
- 文件不存在时返回非 0 并输出错误日志

## 风险与 TODO

- 当前日志模块仍较简单；已支持 SD 卡追加写入，但尚未实现日志轮转、压缩或大小限制
- 当前 SD storage 只提供目录初始化、状态、dashboard 和日志落盘，尚未实现传感器历史数据记录服务
- 当前配置格式只适合少量简单配置项
...
- MMC5603 原始磁场读取已验证，罗盘航向仍需硬铁、软铁和倾斜补偿
- BMP390 软件集成、构建和烧录链路验证已完成，但 I2C ACK、补偿值和长期采样稳定性尚未通过上板验证
- 当前 `outdoor-agent` 是轻量 fbdev framebuffer 仪表盘原型，不代表触摸交互、窗口系统或完整 GUI 已完成；光照、空气质量、电池和信号等展示项暂为 UI 占位/演示指标
...
- ICM42688 固件路径已实现且有历史验证记录，但当前重新接线后初始化失败，需单独排查 VCC/GND、SCL/SDA、AD0、上拉和模块本体
