...
- F407 I2C2 读取 MMC5603 三轴磁场，20 Hz `sensor_magnetometer` 帧已完成上板验证
- F407 I2C2 读取 BMP390 气压和温度，10 Hz `sensor_barometer` 软件路径已实现并通过构建，真实上板验证待完成
...
当前处于 Stage 1：Sensor Integration 与 STM32F407ZG Sensor Hub 协议解析。MP157 板载 ICM20608、F407 侧 MMC5603、F407/MP157 UART 链路均已完成真实上板验证。MMC5603 当前通过 I2C2 地址 `0x30` 采样并以 20 Hz 上报；BMP390 软件路径已完成，启动时探测 `0x77/0x76` 并以 10 Hz 上报，但尚未烧录或上板；ICM42688 当前重新接线后仍处于 Mock fallback，待独立排查。UBLOX-M10 已确认在 `/dev/ttySTM2`、38400 8N1 输出有效 NMEA，但当前无卫星定位。
...
- F407 固件具备 ICM42688 100 Hz 读取和 Mock fallback；该路径曾通过 `status_flags=0x0001` 验证，但当前接线状态为 fallback/error。
- F407 固件能读取 MMC5603 并以约 20 Hz 输出 `sensor_magnetometer`，MP157 可解析 XYZ μT 状态。
- F407 固件 BMP390 软件路径能生成 10 Hz `sensor_barometer`，MP157 可解析气压和温度；真实模块上板验收尚未执行。
- 文件不存在时返回非 0 并输出错误日志
...
- BMP390 软件集成和构建已完成，但 I2C 地址探测、补偿值和长期采样稳定性尚未上板验证
...
- ICM42688 固件路径已实现且有历史验证记录，但当前重新接线后初始化失败，需单独排查
