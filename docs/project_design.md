...
- `data/nmea_edge_cases.txt`: checksum、无效定位和南/西半球坐标边界样例
...
Stage 0.5 使用文件型状态发布作为最小 IPC 原型，默认输出 `runtime/status.txt`。Unix domain socket 状态查询接口已完成方案评估，暂不在 Stage 0 实现，后续在 STM32MP157 Linux 部署阶段继续推进。
