...
当前本机 14/14 CTest、连续 dashboard 一秒受控运行、storage/history/compass 组合验证和 ARMv7 hard-float 全目标交叉构建已通过。真实 MP157 也已完成 60 秒多设备/SD history 冒烟、修复后 20 秒 framebuffer history 频率复验和 30 秒 1 MiB 日志轮转复验；Stage 2 socket/systemd/mock terminal 板端运行、小时级与掉电验证仍待执行。
...
脚本先拒绝并发长测或非 systemd 管理的 Runtime，保存初始 active/enabled 状态，再验证独立 socket self-test、0750/0660 权限、三次状态查询、Agent mock 无/有 context、SIGTERM 退出与 socket 清理、重新启动，以及一次 SIGKILL 后的自动恢复。EXIT/INT/TERM 收尾会恢复初始 systemd 状态并保留 `/tmp/outdoor-agent-stage2-acceptance-*` 报告。没有 `--confirm` 会在任何 systemd 操作前返回 2；脚本不包含上下电、重启整板、烧录或 COM 操作。本轮只完成脚本、静态契约与负向测试，没有在 MP157 执行。
