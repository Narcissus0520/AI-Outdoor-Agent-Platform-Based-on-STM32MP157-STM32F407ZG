...
- `mp157/outdoor-core-service/`: Linux Outdoor Core Runtime，已完成 Stage 0，并在 Stage 1 支持 MCU heartbeat、mock sensor、Mock IMU 协议帧解析、MP157 板载 ICM20608 的读取服务和上板验证、F407 live serial 输入路径，以及最小 MP157 -> F407 `command_ping` 下行命令。
...
当前不包含 UI、HTTP API、AI Agent。当前 Sensor Integration 已完成 MP157 开发板自带 ICM20608 的主控侧读取、状态输出和上板验证；F407 -> MP157 串口链路已按 `F407 UART4 PC10/PC11 <-> MP157 USART3 PD9/PD8` 完成最小上板验证，MP157 侧设备为 `/dev/ttySTM1`。MP157 -> F407 下行方向已实现最小 `ping -> ack`，并已通过 MP157 shell raw 帧方式完成真实接线验证；新版 ARM Runtime 的 `--mcu-command ping` 板端复测仍待更可靠部署方式。
