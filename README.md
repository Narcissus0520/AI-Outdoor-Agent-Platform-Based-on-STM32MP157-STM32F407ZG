...
当前本机 14/14 CTest、连续 dashboard 一秒受控运行、storage/history/compass 组合验证和 ARMv7 hard-float 全目标交叉构建已通过。真实 MP157 也已完成 60 秒多设备/SD history 冒烟、修复后 20 秒 framebuffer history 频率复验、30 秒 1 MiB 日志轮转复验，以及 Stage 2 socket/systemd/mock terminal 生命周期验收；小时级与掉电验证仍待执行。
...
启用 socket 后 Runtime 包含常驻服务，需要 SIGINT/SIGTERM 或 `runtime_run_seconds` 结束。本机协议与非 POSIX 边界测试、Runtime verifier 以及 GNU ARM Linux 9.2.1 全目标交叉链接已通过；真实 MP157 已验证 0750/0660 权限、stale/active 冲突、连续三次查询、受控退出清理和失败重启后的再次查询。
...
纯软件 verifier 已覆盖 unit 依赖、运行目录、SIGTERM、重启上限、沙箱、设备白名单和 live-source 配置，并以四个不安全变体证明检查会失败。真实 MP157 已完成 enable/disable、start/stop/restart、SIGTERM、单次 SIGKILL 自动恢复与初始状态恢复；当前节点权限为 `/dev/ttySTM1/2` root:dialout 0660、`/dev/icm20608` root:root 0600，因此首版继续显式以 root 运行并用 allow-list 收窄能力。

Stage 2.4 已把后续板端联调固化为显式确认的验收入口。部署完成后，只能在用户安排的联调窗口由 root 执行：

```bash
/opt/outdoor-agent/scripts/run_stage2_board_acceptance.sh --confirm
```

脚本先拒绝并发长测或非 systemd 管理的 Runtime，保存初始 active/enabled 状态，再验证独立 socket self-test、0750/0660 权限、三次状态查询、Agent mock 无/有 context、SIGTERM 退出与 socket 清理、重新启动，以及一次 SIGKILL 后的自动恢复。EXIT/INT/TERM 收尾会恢复初始 systemd 状态并保留 `/tmp/outdoor-agent-stage2-acceptance-*` 报告。没有 `--confirm` 会在任何 systemd 操作前返回 2；脚本不包含上下电、重启整板、烧录或 COM 操作。2026-07-21 真实 MP157 第二轮验收全部通过，PID `2086 -> 2177 -> 2225`、`NRestarts 0 -> 1`，最终恢复 `inactive/disabled` 且 socket 不存在；报告目录为 `/tmp/outdoor-agent-stage2-acceptance-IIppv5/`。

## Local Agent 软件边界

Stage 2.3 新增 `AgentRequest`/`AgentResponse` schema v1、可替换 `IAgentBackend`、有界 `LocalAgentService`、确定性 `MockAgentBackend` 和 `outdoor_agent_terminal`。当前 backend 名为 `mock_no_inference`，成功响应固定声明没有执行 AI inference；这只是接口、错误处理和工具链验证，不是本地模型、远程模型、语音助手或任务执行能力。

无 Runtime 依赖的 mock 调用：

```bash
./outdoor_agent_terminal \
  --prompt "Report current outdoor status" \
  --request-id demo-1 --format text
```

后续板端联调时可显式读取只读 Runtime context：

```bash
./outdoor_agent_terminal \
  --prompt "Report current outdoor status" \
  --request-id demo-2 \
  --status-socket /run/outdoor-agent/outdoor_core.sock \
  --format json
```

请求 ID、prompt、Runtime context、backend 名、响应正文和错误均有固定字节上限；无效 schema/ID、空或多行 prompt、超限数据、backend 失败/异常/空响应都会产生稳定拒绝或失败状态。当前同步接口一次只处理一个请求，不宣称可中止未来阻塞 backend；真实模型接入必须另做依赖、ARM 构建、RAM/CPU/存储/温升、离线、许可证、deadline/cancellation 和故障隔离决策。
...
- [ADR-0027: 定义可替换的 Local Agent Backend 边界](docs/adr/0027-define-local-agent-backend-boundary.md)
- [ADR-0028: 使用恢复初始状态的 Stage 2 板端验收脚本](docs/adr/0028-use-state-restoring-stage2-board-acceptance.md)
