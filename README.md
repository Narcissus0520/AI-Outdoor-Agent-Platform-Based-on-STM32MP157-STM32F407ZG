...
当前处于 Stage 0.5，只实现 Outdoor Core Runtime 的工程骨架、NMEA 文件回放、基础配置加载、日志级别控制、Runtime Service 生命周期骨架、最小 NMEA Parser 和文件型 Runtime 状态发布。不包含串口、HTTP API、UI 或 AI 功能。
...
- [x] GNSS 基础数据模型 `GnssFix`
- [x] 最小 NMEA Parser，支持 RMC/GGA 基础字段
- [x] NMEA checksum 校验
- [x] 输出基础定位状态
- [x] 文件型 IPC 状态发布原型
- [x] Runtime 基础运行状态输出
...
主程序先加载 `config/runtime.conf`，再应用命令行覆盖项，随后创建 Runtime Manager 并注册 GNSS 文件回放服务。服务按 `start/run/stop` 生命周期读取 NMEA 文本，解析 RMC/GGA 基础字段，并通过日志模块输出原始 NMEA 与基础定位状态。Runtime 状态会通过 `StatusPublisher` 写入状态文件，后续可扩展为 Unix domain socket 或 FIFO。
...
status_output_path = runtime/status.txt
log_level = info
```
...
当前 Stage 0.5 使用文件型状态发布作为最小 IPC 原型。默认状态文件：
...
项目处于 Stage 0.5。当前只验证 Linux 用户态 C++ Runtime 的最小启动流程、配置加载、日志级别控制、服务生命周期、NMEA 文件回放、基础定位状态输出和 Runtime 状态文件发布，尚未接入真实硬件数据。
...
- [ADR-0004: 实现最小 NMEA Parser](docs/adr/0004-minimal-nmea-parser.md)
- [ADR-0005: 使用文件型状态发布作为最小 IPC 原型](docs/adr/0005-file-status-ipc-prototype.md)
- [ADR-0006: 评估 Unix Domain Socket 状态查询接口](docs/adr/0006-evaluate-unix-domain-status-query.md)
...
- 使用最小 NMEA Parser 输出 GNSS 基础定位状态
- 使用文件型状态发布建立最小 IPC/Runtime 状态输出基础
