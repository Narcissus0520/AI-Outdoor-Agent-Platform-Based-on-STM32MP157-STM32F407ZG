...
- `f407/sensor-hub/`: STM32F407ZG Sensor Hub 软件目录，包含 PC mock C++ 工程，以及 `firmware/` 下的项目自有 C 代码和 STM32CubeMX/HAL 工程。当前固件通过 UART4 PC10/PC11 对接 MP157 USART3，发送 heartbeat、IMU、MMC5603 磁力计和 BMP390 气压计协议帧，并可回复 MP157 下发的 `command_ping`。MMC5603 已完成 I2C2 真实数据上板验证；BMP390 已完成软件集成和构建验证，尚未上板；ICM42688 当前重新接线后仍处于 Mock fallback。
...
MMC5603 与 ICM42688 共用 I2C2：`SCL -> PB10`、`SDA -> PB11`，MMC5603 地址为 `0x30`。固件使用 20 Hz 单次测量并发送 `sensor_magnetometer` 帧；IMU 调度使用固定 10 ms 时间基准，测量结束后可追赶周期。当前 ICM42688 仍回退 Mock IMU，因此 heartbeat 为 `status_flags=0x000E`：MMC5603 ready，同时 IMU fallback/error。

BMP390 同样复用 I2C2。固件启动时依次探测 `0x77` 和 `0x76`，使用 Bosch BMP3 Sensor API v2.0.5 的 64-bit 定点补偿，配置温度/气压 2x 过采样、IIR 系数 3 和 25 Hz ODR，并以 10 Hz 发送 `sensor_barometer` 帧。该路径已通过 F407、MP157 和工具链构建测试，尚未烧录或进行真实传感器上板验证。
