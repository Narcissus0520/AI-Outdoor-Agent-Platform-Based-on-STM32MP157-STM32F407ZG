...
- I2C2：100 kHz，共用总线包含 ICM42688 `0x69`、MMC5603 `0x30` 和 BMP390 `0x77/0x76`。
...
- 已接入 BMP390 地址探测、校准补偿、气压/温度读取；当前仅完成软件和构建验证。
...
USART1 heartbeat 和 IMU 帧曾通过 F407 上板串口抓包验证；当前应用通信口已切换为 UART4，并已通过 MP157 USART3 `/dev/ttySTM1` 上板验证。ICM42688 I2C 数据源曾完成 `status_flags=0x0001` 的历史验证，但当前重新整理接线后初始化失败并回退 Mock IMU。MMC5603 当前真实读取正常。
...
3. `sensor_hub_app_init()` 初始化 Mock IMU fallback，并执行 ICM42688 初始化：
   ...
   - 随后初始化 MMC5603，并初始化 BMP390；BMP390 先探测 `0x77`，再探测 `0x76`。
...
固件会先探测 `0x77`，失败后探测 `0x76`。BMP390 使用 Bosch BMP3 Sensor API v2.0.5 的 64-bit 定点补偿，配置气压/温度 2x 过采样、IIR 系数 3 和 25 Hz ODR。当前尚未烧录或执行真实模块上板验证。
